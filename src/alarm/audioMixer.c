// code used from Brian's example
...
{
	AudioMixer_setVolumn(DEFAULT_VOLUME);
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	if (!audioIsPlaced)
		printf("Audio Mixer: queue is full\n");
}
...
void AudioMixer_setVolumn(int newVolume)
{
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	for (int i = 0; i < MAX_SOUND_BITES; i++) {
		if (soundBites[i].pSound != NULL) {
            ...
            int maxLength = soundBites[i].pSound->numSamples;
            short *mySound = soundBites[i].pSound->pData;
            for (int j = 0; offset < maxLength && j < playbackBufferSize; j++, offset++) {
                int myValue = mySound[offset];

				// int tempValue = (int)playbackBuffer[i] + (int)myValue;
				short tempValue;
                if (playbackBuffer[j] + myValue > SHRT_MAX) {
					tempValue = SHRT_MAX;
				} else if (playbackBuffer[j] + myValue < SHRT_MIN) {
                    tempValue = SHRT_MIN;
                } else {
                    tempValue = playbackBuffer[j] + myValue;
                }

				playbackBuffer[j] = tempValue;
            }
